...
function imageScale:updateOutput(input)
	...
	if torch.isTensor(input) then
		...
		if input:nDimension() == 3 or input:nDimension() == 2 then
			output = image.scale(input, self.height, self.width, self.mode)
			output:typeAs(input)
		elseif input:nDimension() == 4 then
			local batchSize = input:size(1)
			local depth = input:size(2)
			output = torch.Tensor(batchSize, depth, self.height, self.width)
			for i = 1, batchSize do
				output[i] = image.scale(input[i], self.height, self.width, self.mode):typeAs(input)
			end
		end
	else 
		output = {}

...
------------------------------------------------------------------------
local SelfFeedSequencer, parent = torch.class('nn.SelfFeedSequencer', 'nn.AbstractSequencer')

function SelfFeedSequencer:__init(module)
   parent.__init(self)
   if not torch.isTypeOf(module, 'nn.Module') then
      error"SelfFeedSequencer: expecting nn.Module instance at arg 1"
   end
...
function SelfFeedSequencer:updateOutput(inputTable)
   assert(torch.type(inputTable) == 'table', "expecting input table")
   -- Note that the SelfFeedSequencer hijacks the rho attribute of the rnn
...
function SelfFeedSequencer:updateGradInput(inputTable, gradOutputTable)
...
function SelfFeedSequencer:accGradParameters(inputTable, gradOutputTable, scale)
   assert(torch.type(gradOutputTable) == 'table', "expecting gradOutput table")
   assert(#gradOutputTable == #inputTable, "gradOutput should have as many elements as input")
...
function SelfFeedSequencer:accUpdateGradParameters(inputTable, gradOutputTable, lr)
   assert(torch.type(gradOutputTable) == 'table', "expecting gradOutput table")
   assert(#gradOutputTable == #inputTable, "gradOutput should have as many elements as input")
...
function SelfFeedSequencer:remember(remember)
   self._remember = (remember == nil) and 'both' or remember
   local _ = require 'moses'
   assert(_.contains({'both','eval','train','neither'}, self._remember), 
      "SelfFeedSequencer : unrecognized value for remember : "..self._remember)
   return self
end

function SelfFeedSequencer:training()
...
function SelfFeedSequencer:evaluate()
   if self.train ~= false then
...
SelfFeedSequencer.__tostring__ = nn.Decorator.__tostring__